...
THREE.TransformGizmo = function () {
  ...
  this.hide = function () {
    ...
    for ( j in this.handles.children ) this.handles.children[j].visible = false;

    for ( j in this.lines.children ) this.lines.children[j].visible = false;

    for ( j in this.pickers.children ) this.pickers.children[j].visible = false;

    for ( j in this.planes.children ) this.planes.children[j].visible = false;
  ...
  this.show = function () {
    ...
    for ( i in this.handles.children ) this.handles.children[i].visible = true;

    for ( i in this.lines.children ) this.lines.children[i].visible = true;

    for ( i in this.pickers.children ) this.pickers.children[i].visible = showPickers;
...
THREE.TransformGizmoRotate = function () {
  ...
  this.handleGizmos = {
    ...
    X: [
      new THREE.Line( new CircleGeometry(1,'x',0.5), new THREE.TransformGizmoLineMaterial( { color: 0xff0000 } ) ),
    ],
    Y: [
      new THREE.Line( new CircleGeometry(1,'y',0.5), new THREE.TransformGizmoLineMaterial( { color: 0x00ff00 } ) ),
    ],
    Z: [
      new THREE.Line( new CircleGeometry(1,'z',0.5), new THREE.TransformGizmoLineMaterial( { color: 0x0000ff } ) ),
    ],
    E: [
      new THREE.Line( new CircleGeometry(1.25,'z',1), new THREE.TransformGizmoLineMaterial( { color: 0xcccc00 } ) ),
    ],
    XYZE: [
      new THREE.Line( new CircleGeometry(1,'z',1), new THREE.TransformGizmoLineMaterial( { color: 0x787878 } ) ),
    ]
    ...
    if ( _mode == "translate" ) {
      ...
      if ( scope.space == "local" ) {
        ...
        if ( scope.axis == "XYZ") {
          ...
          if ( scope.axis == "X" ) scope.object.scale.x = oldScale.x * ( 1 + point.x );
          if ( scope.axis == "Y" ) scope.object.scale.y = oldScale.y * ( 1 + point.y );
          if ( scope.axis == "Z" ) scope.object.scale.z = oldScale.z * ( 1 + point.z );
